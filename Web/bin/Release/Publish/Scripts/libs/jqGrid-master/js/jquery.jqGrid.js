...
$.fn.jqFilter = function( arg ) {
	...
	return this.each( function() {
		...
		*/
		...
		this.ToStrForGroup = function(group) {
			var s = "(", index;
			if (group.groups !== undefined) {
				for (index = 0; index < group.groups.length; index++) {
					if (s.length > 1) {
					...
					try {
						s += this.ToStrForGroup(group.groups[index]);
					} catch (eg) {alert(eg);}
				}
			}
			...
			if (group.rules !== undefined) {
				try{
					for (index = 0; index < group.rules.length; index++) {
						...
						s += this.ToStrForRule(group.rules[index]);
					}
				} catch (e) {alert(e);}
			}
		...
		this.ToStrForRule = function(rule) {
		...
		this.toUserFriendlyString = function() {
			return this.ToStrForGroup(p.filter);
		};
		this.toString = function() {
			// this will obtain a string that can be used to match an item.
			var that = this;
			function ToStrRule(rule) {
			...
			function ToStrForGroup(group) {
				...
				if (group.groups !== undefined) {
					for (index = 0; index < group.groups.length; index++) {
						...
						s += ToStrForGroup(group.groups[index]);
					}
				}
			...
			return ToStrForGroup(this.p.filter);
		};
