...
var wy = {
    ...
    setTheme: function (index, topIndex) {
        ...
        if (!isNaN(parseInt(topIndex))) {
            for (var i = 0; i < wy.domObj.skinNames.length; i++) {
                wy.domObj.winHeader.removeClass(wy.domObj.skinNames[i]);
            }
            wy.domObj.winHeader.addClass(wy.domObj.skinNames[topIndex]);
    ...
    init: function () {
        ...
        //初始化皮肤
        wy.setTheme(localStorage.getItem("web-theme-leftmenu"), localStorage.getItem("web-theme-top"));
    ...
    tabs: {
        ...
        add: function (obj) {
            ...
            if (flag == 0) {
                ...
                ul.append(flag);
                //创建 iframe
                wy.tabs.createIframe(obj.href);
            }
            wy.tabs.location($(flag));
            wy.tabs.actice($(flag));
        },
        //创建 iframe
        createIframe: function (href) {
            ...
            wy.tabs.loadIframe(iframe, href);
        },
        //关闭选项卡标签
        close: function (event, $this) {
            ...
            $this = $($this);
            wy.tabs.upOrNextLoad($this, function () {
        ...
        removeOtherTab: function () {
            ...
            wy.tabs.location(acticeLi);
            wy.tabs.actice(acticeLi);
        },
        //移除所有选项卡
        ...
        refreshThisTab: function (dom) {
            ...
            wy.tabs.loadIframe(iframe, iframe.attr("src"));
        },
        // iframe 加载
        ...
        return {
            handleFullScreen: function ($this) {
                ...
                if (isFullScreen) {
                    exitFull();
                    isFullScreen = false;
                    $this.find("i").removeClass("icon-size-actual");
                    $this.find("i").addClass("icon-size-fullscreen");
                } else {
                    requestFullScreen();
                    isFullScreen = true;
                    $this.find("i").removeClass("icon-size-fullscreen");
                    $this.find("i").addClass("icon-size-actual");
                }
            },
        };
