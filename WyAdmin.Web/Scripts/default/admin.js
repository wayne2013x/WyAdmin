...
var admin = {
    ...
    //消息框
    alert: function (content, type, title = type) {
        if (type == "警告")
            return admin.layer.alert(content, { icon: 0, title: title });
        else if (type == "成功")
            return admin.layer.alert(content, { icon: 1, title: title });
        else if (type == "错误")
            return admin.layer.alert(content, { icon: 2, title: title });
        else
            return admin.layer.alert(content, {  title: title });
    },
    //提示框
    msg: function (content, type, time = 3500) {
        if (type == "警告")
            return admin.layer.msg(content, { icon: 0, time: time});
        else if (type == "成功")
            return admin.layer.msg(content, { icon: 1, time: time});
        else if (type == "错误")
            return admin.layer.msg(content, { icon: 2, time: time});
        else
            return admin.layer.msg(content, { time: time});
    },
    //询问框
    confirm: function (content, yes, cancel, title = '提醒') {
        admin.layer.confirm(content, { icon: 3, title: title }, yes, cancel);
    },
    //加载
    loading: {
        index: 0,
